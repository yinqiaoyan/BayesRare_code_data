...
umap_df_plot$Group = meta_data_all$cell_type
...
for (vv in tmpLetter){
  if (vv == "b") {
    ...
    ## cellsius (c)
    umap_df_plot$Group = "abundant cells"
    umap_df_plot$Group[umap_df$X %in% cellsius_res$V1] = "rare cells"  
  } else if (vv == "d") {
    ## gapclust (d)
    umap_df_plot$Group = "abundant cells"
    umap_df_plot$Group[umap_df$X %in% gapclust_res$cell] = "rare cells" 
  } else if (vv == "e") {
    ## giniclust (e)
    umap_df_plot$Group = "abundant cells"
    umap_df_plot$Group[umap_df$X %in% giniclust_res$cell_name] = "rare cells" 
  } else if (vv == "f") {
    ## SCISSORS (f)
    umap_df_plot$Group = "abundant cells"
