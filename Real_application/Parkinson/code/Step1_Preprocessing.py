...
## Read in raw data
expr_df = pd.read_csv("../raw_data/raw_gene_expression_matrix.tsv", sep="\t", header=0, index_col=None)
meta_df = pd.read_csv("../raw_data/IPDCO_hg_midbrain_cell.tsv", sep="\t", index_col=0)

expr_df_rm = expr_df.iloc[:, 1:]
meta_df_rm = meta_df.iloc[1:]

expr_df_rm = expr_df_rm.T

expr_logged = np.log1p(expr_df_rm)
...
(meta_df_rm
 .reset_index()       
 .to_csv("../input_data/raw_metadata.tsv", sep="\t", index=False))